...
public class Camera {
    ...
    private float distance = 2.0f, angle = 0.0f, horizontalAngle = 0, verticalAngle = 0;
    private double oldMouseX = 0, oldMouseY = 0, newMouseX, newMouseY;
    private ReferenceList ref;
    public Camera(Vector3f position, Vector3f rotation, ReferenceList ref) {
    ...
    public void update(){
        ...
        float x = (float) Math.sin(Math.toRadians(rotation.getY())) * ref.moveSpeed;
        float z = (float) Math.cos(Math.toRadians(rotation.getY())) * ref.moveSpeed;


        if(!Input.isKeyDown(GLFW.GLFW_KEY_LEFT_SHIFT)) ref.moveSpeed = 0.01f;
        if(Input.isKeyDown(GLFW.GLFW_KEY_LEFT_SHIFT)) ref.moveSpeed = 0.02f;
        ...
        if(Input.isKeyDown(GLFW.GLFW_KEY_SPACE)) position = Vector3f.add(position, new Vector3f(0, ref.moveSpeed ,0));
        if(Input.isKeyDown(GLFW.GLFW_KEY_LEFT_CONTROL)) position = Vector3f.add(position, new Vector3f(0, -ref.moveSpeed ,0));
        ...
        rotation = Vector3f.add(rotation, new Vector3f(-dy * ref.mouseSensitivity, -dx * ref.mouseSensitivity, 0));
    ...
    public void update(GameObject object){
        ...
        if(Input.isButtonDown(GLFW.GLFW_MOUSE_BUTTON_LEFT)) {
            verticalAngle -= dy * ref.mouseSensitivity;
            horizontalAngle += dx * ref.mouseSensitivity;
        }
        if(Input.isButtonDown(GLFW.GLFW_MOUSE_BUTTON_RIGHT)) {
            if(distance > 0) {
                distance += dy * ref.mouseSensitivity;
            } else {
                distance = 0.1f;
            }
        }
